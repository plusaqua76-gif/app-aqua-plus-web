...
@Component({
  ...
  template: `
    <div class="container mx-auto p-6">
      ...
      <div *ngIf="dataHistoryReading.error()" class="mt-4 p-4 bg-red-50 border border-red-200 rounded-md">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">Error al cargar el historial</h3>
            <div class="mt-2 text-sm text-red-700">
              <p>No se pudo cargar el historial de la lectura. Por favor, inténtelo de nuevo.</p>
            </div>
          </div>
        </div>
      </div>

      <div *ngIf="!dataHistoryReading.isLoading() && !dataHistoryReading.error() && (!dataHistoryReading.value()?.response || dataHistoryReading.value()?.response?.length === 0)"
        ...
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-blue-800">Sin historial disponible</h3>
...
export class HistoryReading {
  ...
  historyColumns = signal([
    ...
    { field: 'serial', header: 'Serial Contador', type: 'text' as const },
    ...
    { field: 'usuarioModificacion', header: 'Usuario Modificación', type: 'text' as const },
  ]);
  ...
  readonly transformedHistoryData = computed(() => {
    ...
    return response.response.map(item => ({
      ...item,
      clienteNombre: `${item.contador.cliente.nombre} ${item.contador.cliente.segundoNombre || ''} ${item.contador.cliente.apellido} ${item.contador.cliente.segundoApellido || ''}`.trim(),
  ...
  formatDate(dateString: string): string {
    ...
    try {
      ...
      // OPCIÓN 4: Formato personalizado mi pez
